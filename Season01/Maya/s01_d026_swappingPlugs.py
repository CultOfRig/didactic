...
for compName, mobhaContainer in selectedContainers.iteritems():
    ...
    if toolPanelMobha is not None and toolPanelMobha.isValid():
        ...
        for i in xrange(elemCount):
            ...
            # for k, v in printable.iteritems():
            #     print k, ': ', v

            if disconnect:
                firstCmdArg = activePlugsDict['guidedSource'].partialName(useFullAttributePath=True, includeNodeName=True, useLongNames=True)
                secondCommandArg= activePlugsDict['guided'].partialName(useFullAttributePath=True, includeNodeName=True, useLongNames=True)
                
                m_cmds.disconnectAttr( firstCmdArg, secondCommandArg )
